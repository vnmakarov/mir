/* This file is a part of MIR project.
   ...
   ppc4 call ABI target specific code.
*/
...
static void multiple_load_store (MIR_context_t ctx, struct type *type, MIR_op_t *var_ops, MIR_op_t mem_op, int load_p) {
  ...
  if (type_align (type) == 8) {
    ...
    for (i = 0; size > 0; size--, i++) {
      var_op = var_ops[i / 8];
      if (load_p) {
	insn = MIR_new_insn (ctx, MIR_MOV, op,
			     MIR_new_mem_op (ctx, MIR_T_U8, mem_op.u.mem.disp + i,
					     mem_op.u.mem.base, mem_op.u.mem.index,
					     mem_op.u.mem.scale));
	MIR_append_insn (ctx, curr_func, insn);
	if ((sh = i * 8 % 64) != 0) {
...
static int target_return_by_addr_p (MIR_context_t ctx, struct type *ret_type) {
  ...
  if (((type = fp_homogeneous_type (ctx, ret_type, &n)) == MIR_T_F || type == MIR_T_D) && n <= 8) return FALSE;
...
static op_t target_gen_post_call_res_code (MIR_context_t ctx, struct type *ret_type,
  ...
  if (((type = fp_homogeneous_type (ctx, ret_type, &n)) == MIR_T_F || type == MIR_T_D) && n <= 8) {
    ...
    multiple_load_store (ctx, ret_type, &VARR_ADDR (MIR_op_t, call_ops)[call_ops_start + 2], res.mir_op, FALSE);
  }
  return res;
}
    ...
    for (i = 0; i < n; i++) {
      ...
      MIR_append_insn (ctx, curr_func,
		       MIR_new_insn (ctx, tp_mov (type), MIR_new_reg_op (ctx, reg_var.reg),
				     MIR_new_mem_op (ctx, type,
						     param_decl->offset + (type == MIR_T_F ? 4 : 8) * i,
