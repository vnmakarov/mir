...
typedef enum {
  T_NUMBER = 256, T_CH, T_STR, T_ID, T_ASSIGN, T_DIVOP, T_ADDOP, T_SH, T_CMP, T_EQNE, T_ANDAND,
  T_OROR, T_INCDEC, T_ARROW, T_UNOP, T_DOTS, T_BOOL, T_COMPLEX, T_ALIGNOF, T_ALIGNAS, T_ATOMIC,
  T_GENERIC, T_NO_RETURN, T_STATIC_ASSERT, T_THREAD_LOCAL, T_THREAD, T_AUTO, T_BREAK, T_CASE,
  T_CHAR, T_CONST, T_CONTINUE, T_DEFAULT, T_DO, T_DOUBLE, T_ELSE, T_ENUM, T_EXTERN, T_FLOAT,
  T_FOR, T_GOTO, T_IF, T_INLINE, T_INT, T_LONG, T_REGISTER, T_RESTRICT, T_RETURN, T_SHORT,
  T_SIGNED, T_SIZEOF, T_STATIC, T_STRUCT, T_SWITCH, T_TYPEDEF, T_TYPEOF, T_UNION, T_UNSIGNED,
  T_VOID, T_VOLATILE, T_WHILE, T_EOF,
...
static void check_assignment_types (struct type *left, struct type *right,
...
static struct expr *check_assign_op (node_t r, node_t op1, node_t op2,
				     struct expr *e1, struct expr *e2,
...
static int push_const_val (node_t r, op_t *res) {
  ...
  if (floating_type_p (e->type)) {
    ...
    *res = new_op (NULL, (get_mir_type (e->type) == MIR_T_F
			  ? MIR_new_float_op (ctx, e->u.d_val) : MIR_new_double_op (ctx, e->u.d_val)));
  } else {
    assert (integer_type_p (e->type) || e->type->mode == TM_PTR);
    *res = new_op (NULL, (signed_integer_type_p (e->type)
			  ? MIR_new_int_op (ctx, e->u.i_val) : MIR_new_uint_op (ctx, e->u.u_val)));
  }
  return TRUE;
}
...
static op_t cast (op_t op, MIR_type_t t, int new_op_p) {
  ...
  all_types:
    ...
    if (t == MIR_T_I64) {
      ...
      insn_code = (op_type == MIR_T_I32 ? MIR_EXT32
		   : op_type == MIR_T_U32 ? MIR_UEXT32 : MIR_INSN_BOUND);
  ...
  if (! new_op_p && insn_code == MIR_INSN_BOUND && insn_code2 == MIR_INSN_BOUND)
    return op;
  res = get_new_temp (t == MIR_T_I8 || t == MIR_T_U8 || t == MIR_T_I16 || t == MIR_T_U16
		      ? MIR_T_I64 : t);
...
static void emit_scalar_assign (op_t var, op_t *val, MIR_type_t t, int ignore_others_p) {
  if (var.decl == NULL || var.decl->bit_offset < 0) {
    ...
    emit3 (MIR_AND, temp_op1.mir_op, val->mir_op, MIR_new_uint_op (ctx, mask));
    if (! signed_integer_type_p (var.decl->decl_spec.type)) {
      emit2 (MIR_MOV, temp_op3.mir_op, temp_op1.mir_op);
    } else {
      emit3 (MIR_LSH, temp_op3.mir_op, temp_op1.mir_op, MIR_new_int_op (ctx, 64 - width));
      emit3 (MIR_RSH, temp_op3.mir_op, temp_op3.mir_op, MIR_new_int_op (ctx, 64 - width));
    }
    *val = temp_op3;
    if (var.decl->bit_offset != 0)
      emit3 (MIR_LSH, temp_op1.mir_op, temp_op1.mir_op, MIR_new_int_op (ctx, var.decl->bit_offset));
    if (! ignore_others_p)
...
static void gen_initializer (size_t init_start, op_t var,
  ...
  if (var.mir_op.mode == MIR_OP_REG) { /* scalar initialization: */
	...
	if (data_size < str_len) {
	  ...
	  if (data_size > str_len)
	    MIR_new_bss (ctx, NULL, data_size - str_len);
	}
      } else {
...
static MIR_item_t get_ref_item (node_t def, const char *name) {
  ...
  if (def->code == N_FUNC_DEF
    ...
    return (decl->decl_spec.linkage == N_EXTERN
	    ? MIR_new_export (ctx, name) : MIR_new_forward (ctx, name));
  return NULL;
}
...
static void emit_bin_op (node_t r, struct type *type, op_t res, op_t op1, op_t op2) {
  ...
  if (type->mode == TM_PTR) {  /* ptr +/- int */
    ...
    if (op2.mir_op.mode == MIR_OP_INT || op2.mir_op.mode == MIR_OP_UINT) {
      ...
      emit3 (sizeof (mir_size_t) == 8 ? MIR_MUL : MIR_MULS,
	     temp.mir_op, op2.mir_op, MIR_new_int_op (ctx, type_size (type->u.ptr_type)));
      op2 = temp;
    }
  }
  ...
  if (type->mode != TM_PTR
    ...
    emit3 (sizeof (mir_size_t) == 8 ? MIR_DIV : MIR_DIVS,
	   res.mir_op, res.mir_op, MIR_new_int_op (ctx, type_size (type->u.ptr_type)));
  }
}
...
static op_t gen (node_t r, MIR_label_t true_label, MIR_label_t false_label, int val_p) {
  ...
  float_val:
    res = new_op (NULL, (get_mir_type (((struct expr *) r->attr)->type) == MIR_T_F
			 ? MIR_new_float_op (ctx, ld) : MIR_new_double_op (ctx, ld)));
    break;
  case N_CH:
    ll = r->u.ch;
    goto int_val;
  case N_STR:
    res = new_op (NULL, MIR_new_str_op (ctx, r->u.s)); // ??? what to do with decl and str in initializer
    break;
  case N_COMMA:
 ...
 case N_POST_INC: case N_POST_DEC: {
  ...
  case N_IND: {
	...
	emit3 (MIR_ADD, temp_op.mir_op, temp_op.mir_op, MIR_new_reg_op (ctx, res.mir_op.u.mem.base));
  ...
  case N_FIELD: case N_DEREF_FIELD: {
    ...
    if (r->code == N_FIELD) {
      assert (op1.mir_op.mode == MIR_OP_MEM);
      op1.mir_op = MIR_new_mem_op (ctx, t, op1.mir_op.u.mem.disp + decl->offset,
				   op1.mir_op.u.mem.base,
  ...
  case N_SPEC_DECL: { // ??? export and defintion with external declaration
	...
	if (decl->item == NULL)
	  decl->item = MIR_new_import (ctx, name);
      }
  ...
  case N_FUNC_DEF: { // ?? vararg
    ...
    curr_func = MIR_new_func_arr (ctx, NL_HEAD (declarator->ops)->u.s,
    ...
    if (ns->size != 0) {
      ...
      MIR_append_insn (ctx, curr_func, MIR_new_insn (ctx, MIR_ALLOCA, MIR_new_reg_op (ctx, fp_reg),
  ...
  case N_SWITCH: {
	...
	break;
      ...
      emit3 (MIR_BEQ, MIR_new_label_op (ctx, label),
...
static void gen_mir_protos (void) {
  ...
  for (size_t i = 0; i < VARR_LENGTH (node_t, call_nodes); i++) {
    ...
    func_type->proto_item
	 ...
	 (ctx, buf, ret_type == MIR_T_UNDEF ? 0 : 1, &ret_type,
	  VARR_LENGTH (MIR_var_t, vars), VARR_ADDR (MIR_var_t, vars)));
  }
}
...
#define C(n) case N_##n: return #n;
  switch (code) {
    C (IGNORE) C (I) C (L) C (LL) C (U) C (UL) C (ULL) C (F) C (D) C (LD) C (CH) C (STR) C (ID)
    C (COMMA) C (ANDAND) C (OROR) C (EQ) C (NE) C (LT) C (LE) C (GT) C (GE) C (ASSIGN)
    C (BITWISE_NOT) C (NOT) C (AND) C (AND_ASSIGN) C (OR) C (OR_ASSIGN) C (XOR) C (XOR_ASSIGN)
    C (LSH) C (LSH_ASSIGN) C (RSH) C (RSH_ASSIGN) C (ADD) C (ADD_ASSIGN) C (SUB) C (SUB_ASSIGN)
    C (MUL) C (MUL_ASSIGN) C (DIV) C (DIV_ASSIGN) C (MOD) C (MOD_ASSIGN) C (IND) C (FIELD) C (ADDR)
...
"}\n";
  ...
  if ((ok_p = compile (source_name)) && ! prepro_only_p && (interp_exec_p || gen_exec_p)) {
    ...
    for (func = DLIST_HEAD (MIR_item_t, module->items);
	 func != NULL;
