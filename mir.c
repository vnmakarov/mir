...
typedef enum {
  ...
  TAG_F,
  TAG_D,
  TAG_LD, /* 4, 8, 16 bytes for floating point numbers */
  TAG_REG1,
  TAG_REG2,
  TAG_REG3,
  TAG_REG4, /* Reg string number in 1, 2, 3, 4 bytes */
  TAG_NAME1,
  TAG_NAME2,
  TAG_NAME3,
  TAG_NAME4, /* Name string number in 1, 2, 3, 4 bytes */
  TAG_STR1,
  TAG_STR2,
  TAG_STR3,
  TAG_STR4, /* String number in 1, 2, 3, 4 bytes */
  TAG_LAB1,
  TAG_LAB2,
  TAG_LAB3,
  TAG_LAB4, /* Label number in 1, 2, 3, 4 bytes */
  ...
  TAG_TBLOCK,
  TAG_EOI,
  TAG_EOF, /* end of insn with variable number operands (e.g. a call) or end of file */
...
static void write_op (MIR_context_t ctx, FILE *f, MIR_func_t func, MIR_op_t op) {
  switch (op.mode) {
  case MIR_OP_REG: write_reg (ctx, f, MIR_reg_name (ctx, op.u.reg, func)); break;
  case MIR_OP_INT: write_int (f, op.u.i); break;
  case MIR_OP_UINT: write_uint (f, op.u.u); break;
  case MIR_OP_FLOAT: write_float (f, op.u.f); break;
  case MIR_OP_DOUBLE: write_double (f, op.u.d); break;
  case MIR_OP_LDOUBLE: write_ldouble (f, op.u.ld); break;
  case MIR_OP_MEM: {
    bin_tag_t tag;
  ...
  case MIR_OP_REF: write_name (ctx, f, MIR_item_name (ctx, op.u.ref)); break;
  case MIR_OP_STR: write_str (ctx, f, op.u.str); break;
  case MIR_OP_LABEL: write_lab (f, op.u.label); break;
...
static void write_item (MIR_context_t ctx, FILE *f, MIR_item_t item) {
  ...
  if (item->item_type == MIR_data_item) {
    ...
    for (i = 0; i < data->nel; i++)
      ...
      case MIR_T_LD:
        write_ldouble (f, ((long double *) data->u.els)[i]);
        break;
        /* only ptr as ref ??? */
...
static bin_tag_t read_token (MIR_context_t ctx, FILE *f, token_attr_t *attr) {
  ...
  case TAG_U6:
  case TAG_U7:
  case TAG_U8: attr->u = get_uint (ctx, f, c - TAG_U1 + 1); break;
  case TAG_I1:
  case TAG_I2:
  case TAG_I3:
  case TAG_I4:
  ...
  case TAG_I6:
  case TAG_I7:
  case TAG_I8: attr->i = get_int (ctx, f, c - TAG_I1 + 1); break;
  ...
  case TAG_REG2:
  case TAG_REG3:
  case TAG_REG4: attr->u = get_uint (ctx, f, c - TAG_REG1 + 1); break;
  case TAG_NAME1:
  case TAG_NAME2:
  case TAG_NAME3:
  case TAG_NAME4: attr->u = get_uint (ctx, f, c - TAG_NAME1 + 1); break;
  case TAG_STR1:
  case TAG_STR2:
  case TAG_STR3:
  case TAG_STR4: attr->u = get_uint (ctx, f, c - TAG_STR1 + 1); break;
  case TAG_LAB1:
  case TAG_LAB2:
  case TAG_LAB3:
  case TAG_LAB4: attr->u = get_uint (ctx, f, c - TAG_LAB1 + 1); break;
  case TAG_MEM_DISP:
  case TAG_MEM_BASE:
  case TAG_MEM_INDEX:
  case TAG_MEM_DISP_BASE:
  case TAG_MEM_DISP_INDEX:
  case TAG_MEM_BASE_INDEX:
  case TAG_MEM_DISP_BASE_INDEX:
  case TAG_EOI:
  case TAG_EOF: break;
  case TAG_TI8:
  case TAG_TU8:
  ...
  case TAG_TP:
  case TAG_TV:
  case TAG_TBLOCK: attr->t = (MIR_type_t) (c - TAG_TI8) + MIR_T_I8; break;
...
static int read_operand (MIR_context_t ctx, FILE *f, MIR_op_t *op, MIR_item_t func) {
  ...
  case TAG_U6:
  case TAG_U7:
  case TAG_U8: *op = MIR_new_uint_op (ctx, attr.u); break;
  case TAG_I1:
  case TAG_I2:
  case TAG_I3:
  case TAG_I4:
  ...
  case TAG_I6:
  case TAG_I7:
  case TAG_I8: *op = MIR_new_int_op (ctx, attr.i); break;
  ...
  case TAG_REG2:
  case TAG_REG3:
  case TAG_REG4: *op = MIR_new_reg_op (ctx, to_reg (ctx, attr.u, func)); break;
  case TAG_NAME1:
  case TAG_NAME2:
  case TAG_NAME3:
  case TAG_NAME4: {
  ...
  case TAG_STR2:
  case TAG_STR3:
  case TAG_STR4: *op = MIR_new_str_op (ctx, to_str (ctx, attr.u)); break;
  case TAG_LAB1:
  case TAG_LAB2:
  case TAG_LAB3:
  case TAG_LAB4: *op = MIR_new_label_op (ctx, to_lab (ctx, attr.u)); break;
  case TAG_MEM_DISP:
  case TAG_MEM_BASE:
  case TAG_MEM_INDEX:
  case TAG_MEM_DISP_BASE:
      ...
      if (strcmp (name, "module") == 0) {
        ...
        for (;;) {
          ...
          MIR_new_func_reg (ctx, func->u.func, tag_type (tag),
                            read_name (ctx, f, "wrong local var name"));
        }
      } else {
