...
static inline void mir_bitmap_assert_fail (const char *op) {
  fprintf (stderr, "wrong %s for a bitmap", op); assert (0);
}

#define BITMAP_ASSERT(EXPR, OP)				              \
  (void) ((EXPR) ? 0 : (mir_bitmap_assert_fail (#OP), 0))
...
static inline int bitmap_equal_p (const_bitmap_t bm1, const_bitmap_t bm2) {
  ...
  if (bm1_len > bm2_len) {
    temp_bm = bm1; bm1 = bm2; bm2 = temp_bm;
    temp_len = bm1_len; bm1_len = bm2_len; bm2_len = temp_len;
  }
  addr1 = VARR_ADDR (bitmap_el_t, bm1); addr2 = VARR_ADDR (bitmap_el_t, bm2);
...
static inline bitmap_el_t bitmap_el_max3 (bitmap_el_t el1, bitmap_el_t el2,
					  bitmap_el_t el3) {
  if (el1 <= el2)
...
static inline bitmap_el_t bitmap_el_max4 (bitmap_el_t el1, bitmap_el_t el2,
					  bitmap_el_t el3, bitmap_el_t el4) {
  if (el1 <= el2)
...
static inline int bitmap_op2 (bitmap_t dst, const_bitmap_t src1, const_bitmap_t src2,
  ...
  for (bound = i = 0; i < len; i++) {
    old = dst_addr[i];
    if ((dst_addr[i] = op (i >= src1_len ? 0 : src1_addr[i],
			   i >= src2_len ? 0 : src2_addr[i])) != 0)
      bound = i + 1;
    if (old != dst_addr[i])
      change_p = TRUE;
  }
...
static inline bitmap_el_t bitmap_el_and (bitmap_el_t el1, bitmap_el_t el2) {
  return el1 & el2;
}
...
static inline bitmap_el_t bitmap_el_ior (bitmap_el_t el1, bitmap_el_t el2) {
  return el1 | el2;
}
...
static inline int bitmap_op3 (bitmap_t dst, const_bitmap_t src1,
			      const_bitmap_t src2, const_bitmap_t src3,
  ...
  for (bound = i = 0; i < len; i++) {
    old = dst_addr[i];
    if ((dst_addr[i] = op (i >= src1_len ? 0 : src1_addr[i],
			   i >= src2_len ? 0 : src2_addr[i],
			   i >= src3_len ? 0 : src3_addr[i])) != 0)
      bound = i + 1;
    if (old != dst_addr[i])
      change_p = TRUE;
  }
...
static inline bitmap_el_t bitmap_el_ior_and (bitmap_el_t el1, bitmap_el_t el2,
					     bitmap_el_t el3) {
  return el1 | (el2 & el3);
}

/* DST = SRC1 | (SRC2 & SRC3).  Return true if DST changed.  */
static inline int bitmap_ior_and (bitmap_t dst, bitmap_t src1,
				  bitmap_t src2, bitmap_t src3) {
  return bitmap_op3 (dst, src1, src2, src3, bitmap_el_ior_and);
}

static inline bitmap_el_t bitmap_el_ior_and_compl (bitmap_el_t el1,
						   bitmap_el_t el2,
						   bitmap_el_t el3) {
  return el1 | (el2 & ~el3);
}
...
/* DST = SRC1 | (SRC2 & ~SRC3).  Return true if DST changed.  */
static inline int bitmap_ior_and_compl (bitmap_t dst, bitmap_t src1,
					bitmap_t src2, bitmap_t src3) {
