...
#define DLIST_ASSERT(EXPR, OP, T)				              \
  (void) ((EXPR) ? 0 : (dlist_assert_fail (OP, #T), 0))
...
static inline void DLIST_OP (T, prepend) (DLIST (T) *list, T elem) {          \
  ...
  elem->LINK.prev = NULL; elem->LINK.next = list->head; list->head = elem;    \
}                                                                             \
                                                                              \
static inline void DLIST_OP (T, append) (DLIST (T) *list, T elem) {           \
  ...
  elem->LINK.next = NULL; elem->LINK.prev = list->tail; list->tail = elem;    \
}                                                                             \
                                                                              \
static inline void DLIST_OP (T, insert_before)                                \
  (DLIST (T) *list, T before, T elem) {                                       \
  DLIST_ASSERT (list && before && elem && list->tail, "insert_before", T);    \
  if (before->LINK.prev == NULL) {                                            \
    DLIST_ASSERT (list->head == before, "insert_before", T);                  \
    before->LINK.prev = elem; elem->LINK.next = before;                       \
    elem->LINK.prev = NULL; list->head = elem;                                \
  } else {                                                                    \
    DLIST_ASSERT (list->head, "insert_before", T);                            \
    before->LINK.prev->LINK.next = elem; elem->LINK.prev = before->LINK.prev; \
    before->LINK.prev = elem; elem->LINK.next = before;                       \
  }                                                                           \
}                                                                             \
                                                                              \
static inline void DLIST_OP (T, insert_after)                                 \
  (DLIST (T) *list, T after, T elem) {                                        \
  DLIST_ASSERT (list && after && elem && list->head, "insert_after", T);      \
  if (after->LINK.next == NULL) {                                             \
    DLIST_ASSERT (list->tail == after, "insert_after", T);                    \
    after->LINK.next = elem; elem->LINK.prev = after;                         \
    elem->LINK.next = NULL; list->tail = elem;                                \
  } else {                                                                    \
    DLIST_ASSERT (list->tail, "insert_after", T);                             \
    after->LINK.next->LINK.prev = elem; elem->LINK.next = after->LINK.next;   \
    after->LINK.next = elem; elem->LINK.prev = after;                         \
  }                                                                           \
}                                                                             \
                                                                              \
...
#define DLIST_INSERT_BEFORE(T, L, B, E)                                       \
  (DLIST_OP (T, insert_before) (&(L), (B), (E)))
#define DLIST_INSERT_AFTER(T, L, A, E)                                        \
  (DLIST_OP (T, insert_after) (&(L), (A), (E)))
