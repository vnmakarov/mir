...
static void machinize_call (MIR_insn_t call_insn) {
  ...
  if (call_insn->ops[1].mode != MIR_OP_REG && call_insn->ops[1].mode != MIR_OP_HARD_REG) {
    temp_op = MIR_new_reg_op (gen_new_temp_reg (MIR_T_I64, curr_func_item->u.func));
  ...
  for (size_t i = start; i < nops; i++) {
    ...
    if ((arg_reg = get_arg_reg (type, &int_arg_num, &fp_arg_num, &new_insn_code)) != MIR_NON_HARD_REG) {
      ...
      if (ext_insn != NULL)
	gen_add_insn_before (call_insn, ext_insn);
      ...
      create_new_bb_insns (prev_insn, next_insn);
      call_insn->ops[i] = mem_op;
      mem_size += type == MIR_T_LD ? 16 : 8;
      if (ext_insn != NULL)
	gen_add_insn_after (prev_call_insn, ext_insn);
    }
  }
  if (proto->vararg_p) {
  ...
  if (mem_size != 0) { /* allocate/deallocate stack for args passed on stack */
    ...
    create_new_bb_insns (prev_call_insn, next_insn);
    new_insn = MIR_new_insn (MIR_ADD, _MIR_new_hard_reg_op (SP_HARD_REG),
			     _MIR_new_hard_reg_op (SP_HARD_REG), MIR_new_int_op (mem_size));
    MIR_insert_insn_after (curr_func_item, call_insn, new_insn);
    next_insn = DLIST_NEXT (MIR_insn_t, new_insn);
    create_new_bb_insns (call_insn, next_insn);
  }
}
...
static void gen_mov (MIR_insn_t anchor, MIR_insn_code_t code, MIR_op_t dst_op, MIR_op_t src_op) {
...
static void machinize (void) {
  ...
  for (i = int_arg_num = fp_arg_num = mem_size = 0; i < func->nargs; i++) {
    ...
    if (arg_reg != MIR_NON_HARD_REG) {
      ...
      MIR_prepend_insn (curr_func_item, new_insn);
      create_new_bb_insns (NULL, DLIST_NEXT (MIR_insn_t, new_insn));
    } else {
      /* arg is on the stack */
      ...
      MIR_prepend_insn (curr_func_item, new_insn);
      next_insn = DLIST_NEXT (MIR_insn_t, new_insn);
      create_new_bb_insns (NULL, next_insn);
      mem_size += type == MIR_T_LD ? 16 : 8;
    }
  }
  alloca_p = FALSE;
  for (insn = DLIST_HEAD (MIR_insn_t, func->insns); insn != NULL; insn = next_insn) {
    ...
    if (code == MIR_UI2F || code == MIR_UI2D || code == MIR_UI2LD || code == MIR_LD2I) {
      ...
      get_builtin (code, &proto_item, &func_import_item);
      assert (res_reg_op.mode == MIR_OP_REG && op_reg_op.mode == MIR_OP_REG);
      freg_op = MIR_new_reg_op (gen_new_temp_reg (MIR_T_I64, curr_func_item->u.func));
      ...
      MIR_op_t treg_op = MIR_new_reg_op (gen_new_temp_reg (MIR_T_I64, curr_func_item->u.func));
	...
	if (var.type == MIR_T_F || var.type == MIR_T_D)
      ...
      gen_mov (insn, MIR_MOV, MIR_new_mem_op (MIR_T_U32, 0, va_reg, 0, 1), MIR_new_int_op (gp_offset));
      next_insn = DLIST_PREV (MIR_insn_t, insn);
      gen_mov (insn, MIR_MOV, MIR_new_mem_op (MIR_T_U32, 4, va_reg, 0, 1), MIR_new_int_op (fp_offset));
      /* overflow_arg_area_reg: treg = start sp + 8; mem64[va_reg + 8] = treg */
      new_insn = MIR_new_insn (MIR_ADD, treg_op, _MIR_new_hard_reg_op (BP_HARD_REG),
			       MIR_new_int_op (8 /*ret*/ + start_sp_from_bp_offset));
      gen_add_insn_before (insn, new_insn);
      gen_mov (insn, MIR_MOV, MIR_new_mem_op (MIR_T_I64, 8, va_reg, 0, 1), treg_op);
      /* reg_save_area: treg = start sp - reg_save_area_size; mem64[va_reg + 16] = treg */
      new_insn = MIR_new_insn (MIR_ADD, treg_op, _MIR_new_hard_reg_op (BP_HARD_REG),
			       MIR_new_int_op (start_sp_from_bp_offset - reg_save_area_size));
      gen_add_insn_before (insn, new_insn);
      gen_mov (insn, MIR_MOV, MIR_new_mem_op (MIR_T_I64, 16, va_reg, 0, 1), treg_op);
      gen_delete_insn (insn);
	 ...
	 mov func_reg, func ref; mov flag_reg, <0|1>; call proto, func_reg, res_reg, va_reg, flag_reg */
      ...
      func_reg_op = MIR_new_reg_op (gen_new_temp_reg (MIR_T_I64, curr_func_item->u.func));
      flag_reg_op = MIR_new_reg_op (gen_new_temp_reg (MIR_T_I64, curr_func_item->u.func));
      next_insn = new_insn = MIR_new_insn (MIR_MOV, func_reg_op, MIR_new_ref_op (func_import_item));
      gen_add_insn_before (insn, new_insn);
      new_insn = MIR_new_insn (MIR_MOV, flag_reg_op, MIR_new_int_op ((int64_t) mem_op.u.mem.type));
      gen_add_insn_before (insn, new_insn);
      ops[0] = MIR_new_ref_op (proto_item); ops[1] = func_reg_op;
...
static void isave (MIR_insn_t anchor, int disp, MIR_reg_t hard_reg) {
  gen_mov (anchor, MIR_MOV, _MIR_new_hard_reg_mem_op (MIR_T_I64, disp, SP_HARD_REG, MIR_NON_HARD_REG, 1),
	   _MIR_new_hard_reg_op (hard_reg));
}

static void dsave (MIR_insn_t anchor, int disp, MIR_reg_t hard_reg) {
  gen_mov (anchor, MIR_DMOV, _MIR_new_hard_reg_mem_op (MIR_T_D, disp, SP_HARD_REG, MIR_NON_HARD_REG, 1),
	   _MIR_new_hard_reg_op (hard_reg));
}

static void make_prolog_epilog (bitmap_t used_hard_regs, size_t stack_slots_num) {
  MIR_func_t func;
  ...
  new_insn = MIR_new_insn (MIR_MOV,
			   ...
			   fp_reg_op);
  gen_add_insn_before (anchor, new_insn); /* -8(sp) = bp */
  /* Use add for matching LEA: */
  new_insn = MIR_new_insn (MIR_ADD, fp_reg_op, sp_reg_op, MIR_new_int_op (-8));
  gen_add_insn_before (anchor, new_insn);  /* bp = sp - 8 */
  if (! func->vararg_p) {
    service_area_size = 8;
  } else {
    service_area_size = reg_save_area_size + 8; start = -(int64_t) service_area_size;
    isave (anchor, start, DI_HARD_REG); isave (anchor, start + 8, SI_HARD_REG);
    isave (anchor, start + 16, DX_HARD_REG); isave (anchor, start + 24, CX_HARD_REG);
    isave (anchor, start + 32, R8_HARD_REG); isave (anchor, start + 40, R9_HARD_REG);
    dsave (anchor, start + 48, XMM0_HARD_REG); dsave (anchor, start + 64, XMM1_HARD_REG);
    dsave (anchor, start + 80, XMM2_HARD_REG); dsave (anchor, start + 96, XMM3_HARD_REG);
    dsave (anchor, start + 112, XMM4_HARD_REG); dsave (anchor, start + 128, XMM5_HARD_REG);
    dsave (anchor, start + 144, XMM6_HARD_REG); dsave (anchor, start + 160, XMM7_HARD_REG);
  }
  ...
  new_insn = MIR_new_insn (MIR_SUB, sp_reg_op, sp_reg_op, MIR_new_int_op (block_size + service_area_size));
  gen_add_insn_before (anchor, new_insn); /* sp -= block size + service_area_size */
  ...
  for (i = n = 0; i <= MAX_HARD_REG; i++)
    if (! call_used_hard_reg_p (i) && bitmap_bit_p (used_hard_regs, i)) {
      ...
      new_insn = MIR_new_insn (MIR_MOV,
			       _MIR_new_hard_reg_mem_op (MIR_T_I64, (int64_t) (n++ * 8) - bp_saved_reg_offset,
							 BP_HARD_REG, MIR_NON_HARD_REG, 1),
			       _MIR_new_hard_reg_op (i));
      gen_add_insn_before (anchor, new_insn);  /* disp(sp) = saved hard reg */
    }
  /* Epilogue: */
  anchor = DLIST_TAIL (MIR_insn_t, func->insns);
  new_insn = MIR_new_insn (MIR_ADD, sp_reg_op, fp_reg_op, MIR_new_int_op (8));
  gen_add_insn_before (anchor, new_insn); /* sp = bp + 8 */
  ...
  for (i = n = 0; i <= MAX_HARD_REG; i++)
    if (! call_used_hard_reg_p (i) && bitmap_bit_p (used_hard_regs, i)) {
      new_insn = MIR_new_insn (MIR_MOV, _MIR_new_hard_reg_op (i),
			       _MIR_new_hard_reg_mem_op (MIR_T_I64, (int64_t) (n++ * 8) - bp_saved_reg_offset,
							 BP_HARD_REG, MIR_NON_HARD_REG, 1));
      gen_add_insn_before (anchor, new_insn);  /* hard reg = disp(sp) */
    }
  new_insn = MIR_new_insn (MIR_MOV, fp_reg_op,
			   _MIR_new_hard_reg_mem_op (MIR_T_I64, -8, SP_HARD_REG, MIR_NON_HARD_REG, 1));
  gen_add_insn_before (anchor, new_insn); /* bp = -8(sp) */
}
...
static uint8_t *target_translate (size_t *len) {
  size_t i;
  MIR_insn_t insn;
  const char *replacement;
  ...
  for (insn = DLIST_HEAD (MIR_insn_t, curr_func_item->u.func->insns);
    ...
    if (insn->code == MIR_LABEL) {
      ...
      if (replacement == NULL) {
	fprintf (stderr, "failure:"); MIR_output_insn (stderr, insn, curr_func_item->u.func, TRUE);
      } else {
