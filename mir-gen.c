...
#define EXT(tp)                                              \
  do {                                                       \
    int64_t p;                                               \
    if ((ccp_res = get_2iops (insn, &p, TRUE)) != CCP_CONST) \
      goto non_const;                                        \
    val.uns_p = FALSE;                                       \
    val.u.i = (tp) p;                                        \
  } while (0)
...
#define IOP2S(op)                                             \
  do {                                                        \
    int32_t p;                                                \
    if ((ccp_res = get_2isops (insn, &p, TRUE)) != CCP_CONST) \
      goto non_const;                                         \
    val.uns_p = FALSE;                                        \
    val.u.i = op p;                                           \
  } while (0)
...
#define UOP2S(op)                                             \
  do {                                                        \
    uint32_t p;                                               \
    if ((ccp_res = get_2usops (insn, &p, TRUE)) != CCP_CONST) \
      goto non_const;                                         \
    val.uns_p = FALSE;                                        \
    val.u.u = op p;                                           \
  } while (0)
...
#define IOP3(op)                                                   \
  do {                                                             \
    int64_t p1, p2;                                                \
    if ((ccp_res = get_3iops (insn, &p1, &p2, TRUE)) != CCP_CONST) \
      goto non_const;                                              \
    val.uns_p = FALSE;                                             \
    val.u.i = p1 op p2;                                            \
  } while (0)
...
#define IOP3S(op)                                                   \
  do {                                                              \
    int32_t p1, p2;                                                 \
    if ((ccp_res = get_3isops (insn, &p1, &p2, TRUE)) != CCP_CONST) \
      goto non_const;                                               \
    val.uns_p = FALSE;                                              \
    val.u.i = p1 op p2;                                             \
  } while (0)
...
#define UOP3(op)                                                   \
  do {                                                             \
    uint64_t p1, p2;                                               \
    if ((ccp_res = get_3uops (insn, &p1, &p2, TRUE)) != CCP_CONST) \
      goto non_const;                                              \
    val.uns_p = TRUE;                                              \
    val.u.u = p1 op p2;                                            \
  } while (0)
...
#define UOP3S(op)                                                   \
  do {                                                              \
    uint32_t p1, p2;                                                \
    if ((ccp_res = get_3usops (insn, &p1, &p2, TRUE)) != CCP_CONST) \
      goto non_const;                                               \
    val.uns_p = TRUE;                                               \
    val.u.u = p1 op p2;                                             \
  } while (0)
...
#define IOP30(op)                                                        \
  do {                                                                   \
    if ((ccp_res = get_op (insn, 2, &val)) != CCP_CONST || val.u.i == 0) \
      goto non_const0;                                                   \
    IOP3 (op);                                                           \
  } while (0)
...
#define UOP30(op)                                                        \
  do {                                                                   \
    if ((ccp_res = get_op (insn, 2, &val)) != CCP_CONST || val.u.u == 0) \
      goto non_const0;                                                   \
    UOP3 (op);                                                           \
  } while (0)
...
#define BICMP(op)                                                   \
  do {                                                              \
    int64_t p1, p2;                                                 \
    if ((ccp_res = get_3iops (insn, &p1, &p2, FALSE)) != CCP_CONST) \
      goto non_const;                                               \
    val.uns_p = FALSE;                                              \
    val.u.i = p1 op p2;                                             \
  } while (0)
...
#define BICMPS(op)                                                   \
  do {                                                               \
    int32_t p1, p2;                                                  \
    if ((ccp_res = get_3isops (insn, &p1, &p2, FALSE)) != CCP_CONST) \
      goto non_const;                                                \
    val.uns_p = FALSE;                                               \
    val.u.i = p1 op p2;                                              \
  } while (0)
...
#define BUCMP(op)                                                   \
  do {                                                              \
    uint64_t p1, p2;                                                \
    if ((ccp_res = get_3uops (insn, &p1, &p2, FALSE)) != CCP_CONST) \
      goto non_const;                                               \
    val.uns_p = FALSE;                                              \
    val.u.i = p1 op p2;                                             \
  } while (0)
...
#define BUCMPS(op)                                                   \
  do {                                                               \
    uint32_t p1, p2;                                                 \
    if ((ccp_res = get_3usops (insn, &p1, &p2, FALSE)) != CCP_CONST) \
      goto non_const;                                                \
    val.uns_p = FALSE;                                               \
    val.u.i = p1 op p2;                                              \
  } while (0)
